...
interface VerifyResult {
  destPath: string;
  status: 'match' | 'mismatch' | 'missing-local' | 'missing-remote' | 'error';
  errorCode?: string;
  errorMessage?: string;
}
...
function writeManifest(manifestPath: string, entries: RemoteFile[]) {
  ensureDirFor(manifestPath);
    ...
    try {
      ...
      if (isChanged) {
        if (!dryRun) {
          ...
          const idx = updatedManifest.findIndex(
            (e) => e.repo === entry.repo && e.destPath === entry.destPath,
          );
          if (idx !== -1) {
        ...
        logger.log(
          `${dryRun ? '[dry-run] ' : ''}Updated ${entry.destPath} (${oldSha?.slice(0, 8) ?? 'unknown'} -> ${commitSha.slice(0, 8)})`,
        );
        results.push({
...
function runVerify(
  ...
  for (const entry of manifest) {
    ...
    const tempDir = fsTempDir();
    try {
      const remoteContents = fetchFileAtSha(
        entry,
        entry.commitSha,
        tempDir,
        timeoutMs,
...
function main() {
  ...
  const command: Command = SUBCOMMANDS.includes(firstArg as Command)
    ? (firstArg as Command)
    : 'fetch';
  ...
  if (command === 'verify') {
    ...
    const hasIssues = results.some(
      (r) => r.status === 'mismatch' || r.status === 'missing-local' || r.status === 'error',
    );
    if (hasIssues) {
      process.exitCode = 1;
    }
  ...
  let entries: string[] = [...positionalArgs];
